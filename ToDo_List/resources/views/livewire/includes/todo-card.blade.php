<div  class="todo mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 border-blue-500 hover:shadow" wire:key{{$todo->id}}>
    <div class="flex justify-between space-x-2">
        ...
        <div class="flex items-center space-x-2">
            ...
            <button wire:click="delete({{$todo->id}})" class="text-sm text-red-500 font-semibold rounded hover:text-teal-800 mr-1">
                ...
                @if(session('delete'))
                <span class="text-red-500 text-xs">{{session('delete')}}</span>
                    @endif
            </button>
        </div>
    </div>
